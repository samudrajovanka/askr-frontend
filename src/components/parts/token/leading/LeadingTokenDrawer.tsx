...
import { createLeadingTokenSchema } from "@/endpoints/token/validator";
import { isInvalidField } from "@/lib/helpers/field";
import {
  useCreateTokenLeading,
  useTokenLeadings,
  useUpdateTokenLeading,
} from "@/query/token";
import type {
...
const defaultValues = {
  ...
  description: "",
} as CreateLeadingTokenPayload;

const LeadingTokenDrawer = ({
  open,
  onOpenChange,
  workspaceSlug,
  projectSlug,
  ...
  const createMutation = useCreateTokenLeading(workspaceSlug, projectSlug);
  const updateMutation = useUpdateTokenLeading(workspaceSlug, projectSlug);
  const primitiveTokensQuery = useTokenLeadings(
    workspaceSlug,
    projectSlug,
  ...
  const form = useForm({
    ...
    validators: {
      onChange: createLeadingTokenSchema,
      onSubmit: createLeadingTokenSchema,
    },
  ...
  useEffect(() => {
    if (open) {
      if (editToken) {
        const meta = editToken.meta as LeadingMeta | null;
        form.setFieldValue("name", editToken.name);
        form.setFieldValue("layer", editToken.layer);
        form.setFieldValue("value", meta?.rawValue ?? 1.5);
        form.setFieldValue(
          "unit",
          meta?.unit as CreateLeadingTokenPayload["unit"],
        );
        form.setFieldValue(
          "referenceId",
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange} swipeDirection="right">
      <DrawerContent side="right" className="sm:max-w-md">
        <DrawerHeader>
          <DrawerTitle>
            {isEditing ? "Edit Leading Token" : "New Leading Token"}
          </DrawerTitle>
        </DrawerHeader>
                                ...
                                <Select
                                  ...
                                  onValueChange={(v) =>
                                    unitField.handleChange(
                                      v === ""
                                        ? undefined
                                        : (v as CreateLeadingTokenPayload["unit"]),
                                    )
                                  }
                                >
                                  <SelectTrigger className="w-24 font-mono shrink-0">
                                    <SelectValue placeholder="none" />
                                  </SelectTrigger>
                                  <SelectContent>
                                    <SelectItem value="">unitless</SelectItem>
                                    {leadingUnits.map((u) => (
                ...
                <Field data-invalid={isInvalid}>
                  ...
                  <InputGroup>
                    <InputGroupTextarea
                      ...
                      onBlur={field.handleBlur}
                      onChange={(e) => field.handleChange(e.target.value)}
                      placeholder="Briefly describe what this line height is for"
                      rows={3}
                    />
...
export default LeadingTokenDrawer;
