...
import { createTrackingTokenSchema } from "@/endpoints/token/validator";
import { isInvalidField } from "@/lib/helpers/field";
import {
  useCreateTokenTracking,
  useTokenTrackings,
  useUpdateTokenTracking,
} from "@/query/token";
import type {
...
type TrackingUnit = (typeof trackingUnits)[number];
...
const defaultValues = {
  ...
  value: 0,
  unit: "em" as TrackingUnit,
  referenceId: undefined,
  status: TOKEN_STATUSES.STABLE,
  description: "",
} as CreateTrackingTokenPayload;

const TrackingTokenDrawer = ({
  open,
  onOpenChange,
  workspaceSlug,
  projectSlug,
  ...
  const createMutation = useCreateTokenTracking(workspaceSlug, projectSlug);
  const updateMutation = useUpdateTokenTracking(workspaceSlug, projectSlug);
  const primitiveTokensQuery = useTokenTrackings(
    workspaceSlug,
    projectSlug,
  ...
  const form = useForm({
    ...
    validators: {
      onChange: createTrackingTokenSchema,
      onSubmit: createTrackingTokenSchema,
    },
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange} swipeDirection="right">
      <DrawerContent side="right" className="sm:max-w-md">
        <DrawerHeader>
          <DrawerTitle>
            {isEditing ? "Edit Tracking Token" : "New Tracking Token"}
          </DrawerTitle>
        </DrawerHeader>
                        ...
                        <Field data-invalid={isInvalid} data-required>
                          ...
                          <div className="flex gap-2">
                            ...
                            <form.Field name="unit">
                              {(unitField) => (
                                <Select
                                  value={unitField.state.value ?? "em"}
                                  onValueChange={(v) =>
                                    unitField.handleChange(v as TrackingUnit)
                                  }
                                >
                ...
                <Field data-invalid={isInvalid}>
                  ...
                  <InputGroup>
                    <InputGroupTextarea
                      ...
                      onBlur={field.handleBlur}
                      onChange={(e) => field.handleChange(e.target.value)}
                      placeholder="Briefly describe what this letter spacing is for"
                      rows={3}
                    />
...
export default TrackingTokenDrawer;
